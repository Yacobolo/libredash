...
import { blockStartsForAll, emptyBlocks, emptyTable, preserveKnownCardinality, sameSort, sortedBlockRows, tableConverter } from './block-source'
import {
...
class ReportTable extends LitElement {
  ...
  willUpdate(changedProperties: Map<PropertyKey, unknown>): void {
	...
	if (previousTable) {
	  this.table = preserveKnownCardinality(previousTable as TableSignal, this.table)
	}
  ...
  private rowRangeText(): string {
	if (!this.availableRows) return this.table.totalRowsKnown ? 'No rows' : 'Waiting for row count'
	...
	const total = this.table.totalRowsKnown ? this.table.totalRows.toLocaleString() : '…'
