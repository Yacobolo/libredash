...
CREATE TABLE IF NOT EXISTS refresh_job_runs (
  ...
  retry_of TEXT REFERENCES refresh_job_runs(id) ON DELETE SET NULL,
  status TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS refresh_job_runs_retry_idx
  ON refresh_job_runs(retry_of);
...
CREATE TABLE IF NOT EXISTS query_events (
  ...
  connection_wait_ms INTEGER NOT NULL DEFAULT 0,
  database_ms INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS api_releases_project_created_idx
  ON api_releases(project_id, created_at DESC, id DESC);

CREATE TABLE IF NOT EXISTS api_release_artifacts (
...
CREATE TABLE IF NOT EXISTS api_release_connections (
  release_id TEXT NOT NULL REFERENCES api_releases(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS api_deployment_releases (
...
CREATE TABLE IF NOT EXISTS api_async_events (
