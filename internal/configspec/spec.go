...
var settings = []Setting{
	...
	{Name: "LIBREDASH_BOOTSTRAP_ADMIN_EMAIL", Field: "BootstrapEmail", Type: TypeString, Category: "administration", Scope: "instance initialization", Description: "Email assigned to the initial production administrator.", Example: "admin@example.com", Runtime: true, Lifecycle: "supported", EnvExample: "admin@example.com"},
	...
	{Name: "LIBREDASH_ENVIRONMENT", Field: "Environment", Type: TypeString, Category: "server", Scope: "serve,admin", Description: "Single serving environment permanently bound to this LibreDash instance.", Example: "prod", Runtime: true, Lifecycle: "supported", Commented: true},
	...
	{Name: "LIBREDASH_MAP_ASSET_DIR", Field: "MapAssetDir", Type: TypeString, Default: ".data/map-assets", Category: "assets", Scope: "serve", Description: "Local root containing the verified, content-addressed basemap package.", Example: "/var/lib/libredash/map-assets", Runtime: true, Lifecycle: "supported", Commented: true},
	...
	{Name: "LIBREDASH_MANAGED_DATA_DIR", Field: "ManagedDataDir", Type: TypeString, Category: "managed data", Scope: "serve", Description: "Private local root for managed-data objects, upload staging, and verified runtime views; defaults beneath LIBREDASH_HOME.", Runtime: true, Lifecycle: "supported", EnvExample: "/var/lib/libredash/managed-data", Commented: true},
	...
	{Name: "LIBREDASH_SITE_BASE_URL", Type: TypeString, Category: "site", Scope: "public site", Description: "Externally visible HTTP(S) origin used for canonical URLs, discovery documents, and transport policy.", Example: "https://leapview.dev", Lifecycle: "supported"},
