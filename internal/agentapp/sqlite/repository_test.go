...
func TestRepositoryPersistsConversationRunMessagesAndEvents(t *testing.T) {
	...
	conversation, err := repo.CreateConversation(ctx, agentapp.ConversationInput{
		WorkspaceID:  "test",
	...
	if conversation.Status != agentapp.ConversationStatusActive || conversation.TranscriptJSON != "[]" {
	...
	hidden, err := repo.CreateConversation(ctx, agentapp.ConversationInput{
		WorkspaceID: "test",
	...
	assistantMessage, err := repo.AppendMessage(ctx, agentapp.MessageInput{
		WorkspaceID:    "test",
		PrincipalID:    owner.ID,
		ConversationID: conversation.ID,
		Role:           agentapp.MessageRoleAssistant,
	...
	if len(messages) != 2 || messages[0].Role != agentapp.MessageRoleUser || messages[1].Role != agentapp.MessageRoleAssistant {
	...
	run, err := repo.CreateRun(ctx, agentapp.RunInput{
		WorkspaceID:    "test",
		PrincipalID:    owner.ID,
		ConversationID: conversation.ID,
		RunID:          "run_external",
	...
	if run.Status != agentapp.RunStatusRunning || run.ID != "run_external" {
		t.Fatalf("run = %#v", run)
	}
	eventOne, err := repo.AppendEvent(ctx, agentapp.EventInput{
		WorkspaceID: "test",
		PrincipalID: owner.ID,
		RunID:       run.ID,
		Sequence:    7,
	...
	eventTwo, err := repo.AppendEvent(ctx, agentapp.EventInput{
		WorkspaceID: "test",
		PrincipalID: owner.ID,
		RunID:       run.ID,
		Sequence:    8,
	...
	run, err = repo.FinishRun(ctx, agentapp.RunFinish{
		WorkspaceID:    "test",
		PrincipalID:    owner.ID,
		ConversationID: conversation.ID,
		RunID:          run.ID,
...
func TestRepositoryRejectsInvalidJSON(t *testing.T) {
	...
	if _, err := repo.CreateConversation(ctx, agentapp.ConversationInput{
		WorkspaceID:  "test",
	...
	if _, err := repo.AppendMessage(ctx, agentapp.MessageInput{
		WorkspaceID:    "test",
		PrincipalID:    principal.ID,
		ConversationID: conversation.ID,
		Role:           agentapp.MessageRoleUser,
