...
func TestAgentToolsCommandListsGeneratedTools(t *testing.T) {
	...
	for _, want := range []string{"NAME", "PERMISSION", "list_dashboards", "VIEW_ITEM", "list_assets", "describe_asset", "asset_lineage", "search_workspace", "query_dashboard_visual_data", "query_semantic_dataset", "explain_semantic_query"} {
		if !strings.Contains(output, want) {
