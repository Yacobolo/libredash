...
SCIM synchronizes enterprise users, groups, memberships, and active state into LibreDash. It does not authenticate browser sessions and does not replace LibreDash authorization.
...
Set the tenant URL to the SCIM base and use bearer-token authentication. LibreDash exposes service-provider metadata, schemas, resource types, and CRUD/PATCH operations for users and groups.
...
LibreDash preserves identity and audit continuity during deactivation. Setting `active=false` or deleting a SCIM user disables the principal, removes SCIM group memberships, and revokes sessions and API tokens. It is a soft disable rather than erasing historical attribution.
...
Use the directory as the source of truth for enterprise group membership. Use LibreDash role bindings, grants, and policies as the source of product authorization.
...
- service principals remain managed through the LibreDash access API.
...
Plan rotation with the directory provider. Install a new LibreDash token and update the provider in a controlled window; because the runtime exposes one configured bearer token, coordinate cutover so failed provisioning does not persist unnoticed.

After rotation, trigger a test update and verify it in LibreDash. Revoke the old value in the secret manager and ensure it was not retained in provider diagnostics or operator shell history.

## Monitor provisioning

Alert on repeated SCIM failures, authentication failures, unexpected mass deactivation, membership churn, and directory/provider throttling. Preserve directory audit logs alongside LibreDash logs to correlate a provisioning request with a product access change.
